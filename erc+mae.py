...
metrics_history = {'train_loss': [], 'val_loss': [], 'train_tiae': [], 'train_mse': [], 'val_tiae': [], 'val_mse': []}

# 数据集路径
original_image_dir = '/home/featurize/data/傅里叶原图像'
denoised_image_dir = '/home/featurize/data/傅里叶去噪图像'
...
class DnCNN(nn.Module):
    def __init__(self, depth=50, image_channels=1, use_bnorm=True):
        ...
        self.Normalize = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        ...
        out = x + self.Normalize(self.tail(out))
        return out
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=EPOCHS, validate_every=5):
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_loader:
            ...
            loss = mae_criterion(outputs, clean_imgs)
            loss.backward()
            optimizer.step()
            ...
            train_mse += mse_criterion(outputs, clean_imgs).item()
        
        print(f'Epoch [{epoch+1}/{num_epochs}], Loss: {epoch_loss/len(train_loader)}, {train_tiae / len(train_loader)}, {train_mse / len(train_loader)}')
        ...
        metrics_history['train_mse'].append(train_mse / len(train_loader))
...
def validate_model(model, val_loader, criterion):
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            val_mse += mse_criterion(outputs, clean_imgs).item()
    ...
    metrics_history['val_mse'].append(val_mse / len(val_loader))
    
    print(f'Validation Loss: {cur_loss}, {val_tiae / len(val_loader)}, {val_mse / len(val_loader)}')
