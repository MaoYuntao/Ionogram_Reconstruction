...
metrics_history = {'train_loss': [], 'val_loss': [], 'train_mse': [], 'train_mae': [], 'val_mse': [], 'val_mae': []}
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=EPOCHS, validate_every=5):
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_loader:
            ...
            loss = criterion(noisy_imgs, clean_imgs, outputs)
            loss.backward()
            optimizer.step()
...
def validate_model(model, val_loader, criterion):
    ...
    print(f'Validation Loss: {cur_loss:.6f}')
